...
fn assemble(
    ...
    let deps_copy = deps_map.clone();
    let all_dependents: HashSet<String> = deps_copy.values().cloned().flatten().collect();
    let all_depends_on: HashSet<String> = deps_copy.keys().cloned().collect();
    let mut can_finish: HashSet<String> = all_depends_on.difference(&all_dependents).cloned().collect();
        ...
        for (job, &time_left) in &in_progress.clone() {
            if time_left == 0 {
                ...
                if let Some(dependents) = dependents {
                    ...
                    let new_cfs: HashSet<String> = dependents.difference(&remaining_dependents).cloned().collect();
