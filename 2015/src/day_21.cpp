...
std::pair<int, int> findCheapest(fighter_t boss) {
    ...
    for(stats_t weapon: weapons) {
        for(stats_t armor: armors) {
            for(stats_t ring1: rings) {
                for(stats_t ring2: rings) {
                    if(ring1.name == ring2.name) { continue; }
                    int cost, damageRating, armorRating;
