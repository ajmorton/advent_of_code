...
int distanceAfter(const deer_t& deer, int time) {
...
tuple<int, int> day_14(const string& input) {
    ...
    int maxDistance = 0;
    for(const deer_t& deer: deers) { maxDistance = std::max(maxDistance, distanceAfter(deer, 2503)); }

    for(int t = 1; t <= 2503; t++) {
        int maxDistAtTime = 0;
        for(const deer_t& deer: deers)  { maxDistAtTime = std::max(maxDistAtTime, distanceAfter(deer, t)); }
        for(deer_t& deer: deers) { deer.score += distanceAfter(deer, t) == maxDistAtTime; }
    }

    int maxScore = 0;
    for(const deer_t& deer: deers) { maxScore = std::max(maxScore, deer.score); }
